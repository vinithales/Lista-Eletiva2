...
<body class="container">
    ...
    <form method="post" action="/produtos/{{$produto->id}}">
        ...
        <div class="mb-3">
            <label for="" class="form-label">Informe a descrição</label>
            <textarea id="descricao" name="descricao" value="{{$produto->descricao}}"class="form-control" rows="4">{{ trim($produto->descricao)}}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <select id="categoria_id" name="categoria_id" class="form-select" required="">
                @foreach ($categorias as $c)
                    <option value="{{ $c->id }}" {{$produto->categoria_id == $c->id ? "selecionado" : "" }}>  
                        {{$c->nome}}
                    </option>
                @endforeach
            </select>
